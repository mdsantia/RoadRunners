...
export default function AttractionsList() {
  ...
  }, [tripDetails.stops, tripDetails && tripDetails.allStops]);

  useEffect(() => {
    if (tripDetails) {
      setAllAttractions(tripDetails.allStops);
      selectedAttractions.forEach((stop) => {
        if (stop.category !== 'start' && stop.category !== 'end' && !tripDetails.stops.some(item => item.id === stop.id)) {
          var index = selectedAttractions.indexOf(stop);
